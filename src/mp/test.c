...
testvecdigmuladd(char *str, mpdigit d)
{
	...
	if(loops > 1)
		printf("%lld ns for a %d*%d vecdigmul\n", (nsec()-now)/loops, b->top*Dbits, Dbits);
	mpnorm(b2);
	printf("0 + %s * %x = %s\n", S(b), d, S(b2));
...
testinvert(mpint *x, mpint *y)
{
	...
	for(i = 0; i < loops; i++)
		mpinvert(x, y, r);
	if(loops > 1)
		printf("%lld µs for a %d in %d invert\n", (nsec()-now)/(1000*loops),
			x->top*Dbits, y->top*Dbits);
	printf("%s**-1 mod %s = %s\n", S(x), S(y), S(r));
	mpmul(r, x, d1);
	mpmod(d1, y, d2);
...
testexp(char *base, char *exp, char *mod)
{
	...
	for(i = 0; i < loops; i++)
		mpexp(b, e, m, res);
	if(loops > 1)
		printf("%ulldµs for a %d to the %d bit exp\n", (nsec()-now)/(loops*1000),
			b->top*Dbits, e->top*Dbits);
	if(m != nil)
		printf("%s %s ^ %s mod p %s\n", S(b), S(e), S(m), S(res));
	else
		printf("%s %s ^ p %s\n", S(b), S(e), S(res));
	mpfree(b);
	mpfree(e);
...
testgcd(void)
{
	...
	for(i = 0; i < loops; i++){
		...
		mpadd(t1, t2, t2);
		if(mpcmp(d, t2) != 0)
			printf("%d gcd %s*%s+%s*%s != %B\n", i, S(a), S(x), S(b), S(y), S(d));
//		else
//			printf("%d euclid %s*%s+%s*%s == %B\n", i, S(a), S(x), S(b), S(y), S(d));
		mpfree(a);
		mpfree(b);
	}
