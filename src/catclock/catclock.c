...
void	init(void);
Bitmap	*draweye(double);
Bitmap	*drawtail(double);
Bitmap	*eballoc(Rectangle, int);
int	fillpoly(Bitmap *, Point [], int, Windrule, int, Fcode);
void	drawpoly(Bitmap *, Point [], int, int, Fcode);

Bitmap *
eballoc(Rectangle r, int ldepth)
{
	Bitmap *b=balloc(r, ldepth);
	if(b==0){
...
{
	catoffs.x=(r.min.x+r.max.x-CATWID)/2;
	catoffs.y=(r.min.y+r.max.y-CATHGT)/2;
	if(!ptinrect(catoffs, r)) fprint(2, "catclock: window too small, reshape!\n");
	redraw=1;
	screen.r=r;
}
...
//	thread(testthread, 0);
	ereshaped(screen.r);
	cat=eballoc(Rect(0, 0, CATWID, CATHGT), 0);
	...
	for(;;){
		if(ecanmouse()) emouse();	/* don't get reshape events without this! */
		drawclock();
		bflush();
		sleep(UPDATE);
	}
...
void drawhand(int length, int width, double theta){
	...
	Point vhand[3];
	vhand[0]=add(catoffs, Pt(CATWID/2+round(length*s), CATHGT/2-round(length*c)));
	vhand[1]=add(catoffs, Pt(CATWID/2-round(ws+wc), CATHGT/2+round(wc-ws)));
	vhand[2]=add(catoffs, Pt(CATWID/2-round(ws-wc), CATHGT/2+round(wc+ws)));
	fillpoly(&screen, vhand, 3, Nonzero, WHITE, S);
	drawpoly(&screen, vhand, 3, BLACK, S);
}
/*
 * draw a cat tail, t is time (mod 1 second)
 */
Bitmap *drawtail(double t){
	Bitmap *bp;
	double theta=.4*sin(t+3.*PIO2)-.08;	/* an assymetric tail leans to one side */
	double s=sin(theta), c=cos(theta);
	Point rtp[NTP];
	int i;
	bp=eballoc(Rect(0, 0, TAILWID, TAILHGT), screen.ldepth);
	for(i=0;i!=NTP;i++)
		rtp[i]=add(Pt(tp[i].x*c+tp[i].y*s, -tp[i].x*s+tp[i].y*c), toffs);
	fillpoly(bp, rtp, NTP, Nonzero, BLACK, S);
	return bp;
}
/*
 * draw the cat's eyes, t is time (mod 1 second)
 */
Bitmap *draweye(double t){
	Bitmap *bp;
	double u;
	...
	if(eyes==0){
		eyes=eballoc(Rect(0, 0, eyes_width, eyes_height), 0);
		wrbitmap(eyes, eyes->r.min.y, eyes->r.max.y, eyes_bits);
	}
	bp=eballoc(eyes->r, screen.ldepth);
...
drawclock(void){
	...
	tm.hour%=12;
	if(redraw || tm.min!=otm.min || tm.hour!=otm.hour){
		if(redraw){
...
int fillpoly(Bitmap *b, Point vert[], int nvert, Windrule w, int v, Fcode f){
