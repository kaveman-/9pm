...
newtmp(int num)
{
	...
	i = getpid();
	do
		sprint(tempnam, "%s/%d%.4s%dsam", gettmp(), num, getuser(), i++);
	while(access(tempnam, 0) == 0);
	fd = create(tempnam, ORDWR|OCEXEC|ORCLOSE, 0600);
	if(fd < 0){
		remove(tempnam);
		fd = create(tempnam, ORDWR|OCEXEC|ORCLOSE, 0600);
	}
